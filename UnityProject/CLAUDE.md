...
除 `tengine-dev` skill（框架 API 规范）和 `.claude/memory`（操作约束）外，第三层知识源：

**Obsidian vault**（`E:\Notes\agent`，经 obsidian MCP）—— 存**领域原理 / 设计推导 / 调研长文**（区别于 memory 的结论、skill 的 API 规范）。

| 知识源 | 存什么 | 何时查 |
|---|---|---|
| `tengine-dev` skill | TEngine 框架 API 规范 | 写 TEngine 代码前（L2-L4，唯一权威源） |
| `.claude/memory` | 操作约束 / 踩坑结论 / 项目状态 | 自动注入，避免再犯 |
| **Obsidian vault** | 原理 / 设计 / 调研长文 | 设计性任务、选型调研、踩坑完整复盘 |

**查 vault**（obsidian MCP 工具，扁平结构，靠 frontmatter 字段检索）:
- `search_query({"==":[{"var":"frontmatter.type"},"research"]})` — 按 type 查（research/pitfall/knowledge/blueprint）
- `search_simple("关键词")` — 全文检索
- `vault_read(path)` — 读一篇，顺 `[[ ]]` 链接扩上下文
- 入口 `_MOC.md` → 主题 MOC（`_MOC-AI` / `_MOC-Unity` / `_MOC-HOK`）

> 分工：**"为什么这么设计 / 之前调研过 / 完整排查链路"** 优先 vault；**"这个 API 怎么用"** 查 skill；**"别踩坑"** 看 memory。三者互补不竞争。
